...
use App\Mail\PagarOrden;
use App\Mail\RetirarGarantia;
...
class OrdenDeServicioController extends Controller
{
    ...
    public function cambiarEstado(OrdenDeServicio $nroOrdenDeServicio, CambiarEstadoRequest $request)
    {
        ...
        $this->historialEstado->save(); //!isset($ordenDeServicio->nro_orden_anterior)
        
        // if ($request->nombre_estado == "No reparado") {
        //     $estadoAnterio = true;
        //     // dd($estadoAnterio);
        // }
        if ($request->nombre_estado == $this->estado::LISTOPARAENTREGA && $this->ordenDeServicio->nro_orden_anterior == null && $this->ordenDeServicio->importe_reparacion != 0) {
            $url = action([PagoController::class, 'preview'], ['orden_de_servicio' => $this->ordenDeServicio->nro]);
            Mail::to($this->ordenDeServicio->cliente->email)->send(new PagarOrden($this->ordenDeServicio->cliente, $this->ordenDeServicio, $url));
        }elseif($request->nombre_estado == $this->estado::LISTOPARAENTREGA && $this->ordenDeServicio->nro_orden_anterior != null || ($request->nombre_estado == $this->estado::LISTOPARAENTREGA && $this->ordenDeServicio->importe_reparacion == 0)){
            Mail::to($this->ordenDeServicio->cliente->email)->send(new RetirarGarantia($this->ordenDeServicio->cliente, $this->ordenDeServicio));
        }
        $this->ordenDeServicio = $this->ordenDeServicio->where('nro', $this->ordenDeServicio->nro)->with('historico_estado', 'celular', 'empleado:dni,nombre,apellido', 'cliente:dni,nombre,apellido,numero_de_telefono')->first();
    ...
    public function store(RequestSaveOrdenDeServicio $request)
    {
        ...
        try {
            ...
            // Estado inicial de un pago
            $request = ['estado' => 'Sin procesar', 'correo' => '', 'nro_orden_de_servicio' => $this->ordenDeServicio->nro];
            Pago::create($request);
