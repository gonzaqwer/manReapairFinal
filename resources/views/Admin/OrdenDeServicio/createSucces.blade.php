            ...
            @if ($ordenDeServicio->ordenSiguiente != null)
                <div class="alert alert-info text-center" role="alert">
                    <p><span class="fw-bold">Esta orden tuvo un reingreso por garantia.</span><br>
                        ...
                        <a href="{{ route('admin.ordenDeServicio.ver', ['nroOrdenDeServicio' => $ordenDeServicio->ordenSiguiente->nro]) }}"
                        ...
                        <div class="card mt-2">
                            <div class="card-body bg-light">
                                ...
                                <p class="card-text">{{ $estado->pivot->created_at->format('d/m/Y H:i') }}
                                    ...
                                    @if (!isset($ordenDeServicio->nro_orden_anterior))
                                        @if ($estado->nombre == 'Listo para entrega' && $ordenDeServicio->ultimoPago->estado == 'Pagado')
                                            <h2 style="color:green">Pagada</h2>
                                        {{-- @elseif($estado->nombre == 'Listo para entrega' && $ordenDeServicio->ultimoPago->estado != 'Pagado')
                                            <h2 style="color:red">Sin pagar</h2> --}}
                                            {{-- <a class="btn btn-success"
                                                href="{{ route('pago.preview', $ordenDeServicio) }}">Pagar</a> --}}
                                        @endif
                                    @elseif($estado->nombre == 'Listo para entrega')
                                        <h2 style="color : orange">Reingreso</h2>
                                    @endif
