            ...
            <div class="row justify-content-end">
                <div class="col-12 col-md-2">
                    <select name="campoBusqueda" class="form-select form-select-sm" aria-label=".form-select-sm example">
                        <option {{ old('campoBusqueda') == '' ? 'selected' : '' }} value="" selected>Seleccionar campo
                        </option>
                        <option {{ old('campoBusqueda') == 'nro' ? 'selected' : '' }} value="nro">Nro</option>
                        <option {{ old('campoBusqueda') == 'imei' ? 'selected' : '' }} value="imei">IMEI</option>
                        <option {{ old('campoBusqueda') == 'nombre_marca' ? 'selected' : '' }} value="nombre_marca">Marca
                        </option>
                        <option {{ old('campoBusqueda') == 'nombre_modelo' ? 'selected' : '' }} value="nombre_modelo">Modelo
                        </option>
                        <option {{ old('campoBusqueda') == 'nombre' ? 'selected' : '' }} value="nombre">Cliente</option>
                        <option {{ old('campoBusqueda') == 'created_at' ? 'selected' : '' }} value="created_at">Fecha de
                            ingreso</option>
                        <option {{ old('campoBusqueda') == 'nombre_estado' ? 'selected' : '' }} value="nombre_estado">Estado
                            actual</option>
                    </select>
                </div>
                <div class="col-12 col-md-2">
                    <input value="{{ old('valorBusqueda') }}" name="valorBusqueda" type="text"
                        class="form-control form-control-sm" placeholder="Valor de busqueda" aria-label="Valor a buscar">
                ...
                @foreach ($ordenesDeServicios as $orden)
                    <tr>
                        ...
                        <td>
                            ...
                            @if (isset($orden->ultimoPago))
                                @if (!isset($orden->historico_estado[2]->nombre))
                                    {{ $orden->ultimoPago->estado }}
                                @endif
                                @if (isset($orden->historico_estado[2]->nombre))
                                    @if ($orden->historico_estado[2]->nombre != 'En reparacion')
                                        {{ $orden->historico_estado[2]->nombre }} 
                                    @else
                                    {{ $orden->ultimoPago->estado }} 
                                    @endif
                                @endif
                        ...
                        <td>
                            {{-- {{dd($orden->historico_estado)}}
                            {{($orden->historico_estado)}} --}}
                            @if ($orden->estado_actual != 'Entregado')
                                <a href="{{ route('admin.ordenDeServicio.cambiarEstado', ['nroOrdenDeServicio' => $orden->nro]) }}"
                            ...
                            @endif
                            <a href="{{ route('admin.ordenDeServicio.ver', ['nroOrdenDeServicio' => $orden->nro]) }}"
                                class="text-black-50" data-bs-toggle="tooltip" data-bs-placement="left"
