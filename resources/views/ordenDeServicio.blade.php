            ...
            <div class="col-4 offset-1">
                ...
                <div class="card border-1 border-dark px-4">
                    <ul class="list-unstyled">
                        ...
                        {{-- <li class="mt-4 fw-lighter fw-bold">Email</li>
                        <li class="fw-lighter">{{ $ordenDeServicio->cliente }}</li> --}}
                    ...
                    @foreach ($ordenDeServicio->historico_estado as $key => $estado)
                        <div class="card mt-2">
                            <div class="card-body bg-light">
                                ...
                                {{$estado->nombre}}
                                x --}}
                                @if ($estado->nombre == 'Listo para entrega' && $ordenDeServicio->importe_reparacion != 0)
                                    @if ($ordenDeServicio->ultimoPago->estado == 'Sin procesar' || $ordenDeServicio->ultimoPago->estado == 'Rechazado')
                                        <a class="btn btn-success"
                                            href="{{ route('pago.preview', $ordenDeServicio) }}">Pagar</a>
                                    @endif
                                    @if ($estado->nombre == 'Listo para entrega' && $ordenDeServicio->ultimoPago->estado == 'Pagado')
                                        <h2 style="color:green">Pagada</h2>
                                    @endif
                                @elseif($estado->nombre == 'Listo para entrega' && $ordenDeServicio->importe_reparacion == '0')
                                    {{-- <h2 style="color: orange">Garantia</h2> --}}
                                @endif
